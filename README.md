...
- Текстовые PDF и таблицы в PDF разбираются через `pdfplumber` / `pypdf`.
- **Сканы** (фото/скан без текстового слоя, как многие КП от поставщиков) обрабатываются через **OCR** (`pymupdf` + `tesseract-ocr-rus` в Docker-образе).
- Для сложных строительных ведомостей OCR сначала читает выделенные столбцы (`наименование`, `количество`), затем склеивает строки в позиции.
- Диапазоны колонок можно подстроить в `.env`: `OCR_NAME_COL_RANGE`, `OCR_QTY_COL_RANGE`, `OCR_TABLE_Y_RANGE` (доли ширины/высоты страницы).
- В UI есть админ-вкладка `OCR калибровка`: можно выбрать ID заявки, листать страницы многостраничного PDF и визуально настраивать рамки колонок перед сохранением.
- Если парсинг не извлёк ни одной позиции, заявка получит статус `parse_status=failed` (раньше подставлялись демо-данные — это убрано).
- Для сложных КП надёжнее загрузить **XLSX/CSV** или вставить позиции **текстом** в UI.

Диагностика PDF на сервере: `python scripts/analyze_pdf.py storage/uploads/<файл>.pdf`
